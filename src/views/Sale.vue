...
<script>
import SaleFilters from '../components/Filters/SaleFilters.vue';
...
export default {
    ...
    const getProducts = async () => {
      ...
      try {
        await db.collection('products').get().then((querySnapshot) => {
          ...
          products.value = resultProducts.filter(product => product.type === 'sale');
          loader.value = true
        })
      } catch (error) {
  ...
  components: { SaleFilters, AppProductCard, AppLoader }
}
</script>
